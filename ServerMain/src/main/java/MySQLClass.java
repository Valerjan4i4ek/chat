...
public class MySQLClass {
        ...
        try{
            ...
            try{
                ...
                st.executeUpdate("CREATE TABLE IF NOT EXISTS chat.privateMessages " +
                        "(id INT NOT NULL, lustTimeClientUpdate BIGINT NOT NULL, message VARCHAR(128) NOT NULL, " +
    ...
    public List<PrivateMessage> checkPrivateMessage(String user){
        ...
        try{
            ...
            try{
                conn = getConnection("chat");
                String query = "SELECT * FROM privateMessages WHERE userTaker = ?";
                ps = conn.prepareStatement(query);
                ps.setString(1, user);
                rs = ps.executeQuery();
